...
export function SEOPanel({
    ...
    // Use props directly with derived state pattern
    const autoGeneratedSlug = !slug && title
        ? title.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/(^-|-$)+/g, "")
        : slug;
    
    const [localSlug, setLocalSlug] = useState(autoGeneratedSlug);
    ...
    // Sync slug when it's auto-generated
    useEffect(() => {
        if (!slug && title && autoGeneratedSlug) {
            onSlugChange(autoGeneratedSlug);
        }
    }, [autoGeneratedSlug, slug, title, onSlugChange]);
