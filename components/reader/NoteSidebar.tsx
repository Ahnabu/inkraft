                                ...
                                >
                                    <div className="p-3 space-y-2">
                                        <textarea
                                            ...
                                            placeholder="Add a general note for this post..."
                                        ...
                                        <div className="flex justify-end gap-2">
                                            <Button size="sm" variant="ghost" onClick={() => setIsAddingNote(false)} className="h-7 text-xs">Cancel</Button>
                                            <Button size="sm" onClick={handleAddNote} disabled={!newNoteContent.trim()} className="h-7 text-xs">Add Note</Button>
                                        </div>
                                    </div>
                                </motion.div>
                            )}
                        ...
                        <div className="flex-1 overflow-y-auto p-4 space-y-3 custom-scrollbar">
                            {loading ? (
                                ...
                                <div className="text-center text-muted-foreground py-8 text-sm bg-muted/20 rounded-lg border border-border/50 p-4">
                                    ...
                                    <p>No notes yet.</p>
                                    <p className="mt-1 text-xs">Highlight text to add a private note.</p>
                                </div>
                            ) : (
                                notes.map((note) => (
                                    <motion.div
                                        ...
                                        {editingNoteId === note._id ? (
                                            <div className="space-y-2">
                                                ...
                                                <div className="flex gap-2 justify-end">
                                                    <Button size="sm" variant="ghost" onClick={() => setEditingNoteId(null)} className="h-7 text-xs">Cancel</Button>
                                                    <Button size="sm" onClick={() => handleEditSave(note._id)} className="h-7 text-xs">Save</Button>
                                                </div>
                                            </div>
                                        ) : (
                                            <>
                                                ...
                                                >
                                                    <div className="flex items-center justify-between">
                                                        <span className="text-[10px] text-muted-foreground font-medium bg-muted/50 px-1.5 py-0.5 rounded">
                                                            {note.paragraphId ? "Contextual Note" : "General Note"}
                                                        </span>
